...
def generate_act_obj_desc(act_game_object, act_desp, persona): 
  ...
  while True:
    ...
    if x: 
      ...
      print ("ERROR: <generate_act_obj_desc> failed. Retrying...")
      continue
...
def generate_battle_summary(persona, battle): 
  battle_summary = run_gpt_prompt_summarize_battle(persona, battle)[0]
  return battle_summary

def generate_decide_to_fight(init_persona, target_person, retrieved):
  x = run_gpt_prompt_decide_to_fight(init_persona, target_person, retrieved)[0]
  if debug: print ("GNS FUNCTION: <generate_decide_to_fight>")
  if x == "yes":
    return True
  else:
    return False
...
def _should_react(persona, retrieved, personas): 
  ...
  def lets_fight(init_persona, target_persona, retrieved):
    if (not target_persona.scratch.act_address 
        or not target_persona.scratch.act_description
        or not init_persona.scratch.act_address
        or not init_persona.scratch.act_description
        or init_persona.scratch.team == target_persona.scratch.team): 
      return False
    ...
    if (target_persona.scratch.fighting_with 
      or init_persona.scratch.fighting_with): 
      return False

    if (target_persona.name in init_persona.scratch.fighting_with_buffer): 
  ...
  if persona.scratch.chatting_with or len(persona.scratch.fighting_with) > 0: 
    return False
...
def _fight_react(maze, persona, focused_event, reaction_mode, personas): # TODO: need to debug
  ...
  if curr_time.second != 0: 
    temp_curr_time = curr_time + datetime.timedelta(seconds=60 - curr_time.second)
    fighting_end_time = temp_curr_time + datetime.timedelta(minutes=inserted_act_dur)
  else: 
    fighting_end_time = curr_time + datetime.timedelta(minutes=inserted_act_dur)
  ...
  for role, p in [("init", init_persona), ("target", target_persona)]: 
    ...
    act_obj_event = (None, None, None)
    _create_react(p, inserted_act, inserted_act_dur,
      act_address, act_event, fighting_with, battle, fighting_with_buffer, fighting_end_time,
      act_pronunciatio, act_obj_description, act_obj_pronunciatio, 
      act_obj_event, act_start_time)

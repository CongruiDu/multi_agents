...
def run_gpt_prompt_go_to_bed_hour(persona, test_input=None, verbose=False):
  """
  Given the persona, returns an integer that indicates the hour when the 
  persona goes to bed.  
  ...
  INPUT: 
    persona: The Persona class instance 
  OUTPUT
    integer for the go to bed hour.
  """
  def create_prompt_input(persona, test_input=None): 
    if test_input: return test_input
    prompt_input = [persona.scratch.get_str_iss(),
  ...
  prompt_template = "persona/prompt_template/v2/go_to_bed_hour_v1.txt"
  prompt_input = create_prompt_input(persona, test_input)
  prompt = generate_prompt(prompt_input, prompt_template)
  fail_safe = get_fail_safe()
  output = safe_generate_response(prompt, gpt_param, 5, fail_safe,
  ...
  return output, [output, prompt, gpt_param, prompt_input, fail_safe]
def run_gpt_prompt_decision(persona,retrieved):
  def __func_clean_up(gpt_response, prompt=""):
  ...
  def create_prompt_input(persona, test_input=None):
    ...
    prompt_input += [persona.scratch.curr_time.strftime("%I:%M %p")]
  ...
  gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 50, 
               ...
               "frequency_penalty": 0, "presence_penalty": 0, "stop": ["\n"]}
  prompt_template = "persona/prompt_template/v2/make_decision_v1.txt"
...
def run_gpt_prompt_next_plan(persona, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    """
    Cleans up the GPT response based on a provided prompt and ensures it is in a usable format.
    
    Args:
        gpt_response (str): The response generated by GPT.
        prompt (str): The prompt or context used to guide the response. Default is an empty string.
    
    Returns:
        list: A cleaned-up and transformed version of the GPT response.
    """
    if not gpt_response:
        return ["No response to clean up."]
    ...
    result = [
        f"{event} from {current_time} to {time}"
...
def run_gpt_prompt_decide_to_fight(persona, target_persona, retrieved,test_input=None, 
                                       verbose=False): 
  def create_prompt_input(init_persona, target_persona, retrieved, 
                          test_input=None): 
...
def run_gpt_prompt_summarize_battle(persona, battle, test_input=None, verbose=False): 
  def create_prompt_input(battle, test_input=None): 
    convo_str = ""
    for row in battle: 
      convo_str += f'{row[0]}: "{row[1]}"\n'
  ...
  def get_fail_safe(): 
    return "battle with a housemate"
  ...
  print ("sfsefdewfdvdsdvfvdi;ldfj as DEBUG 1123") ########
  gpt_param = {"engine": "gpt-3.5-turbo-instruct", "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
